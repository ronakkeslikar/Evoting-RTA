        ...
        public async Task<DataTable> ROMUpload_Details(FJC_ROMUpload fjc_ROMUpload,string Token)
        {
            //validation job
                ...
                { Validate_ROM val_ROM=new Validate_ROM();
                    bool isValid= val_ROM.Validate_File(dt1.Rows[0]["url"].ToString()); //("C:\\evoting\\RTA\\ROM\\2020-09-18\\20200918-112617505-CDSL_FileUpload_128_ROM_ANUH_PHARMA_LTD_13220201.txt");//(dt1.Rows[0]["url"].ToString()); 
                    if(isValid)
                    {
